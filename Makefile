...
#
# LLVM_INCLUDE_PATH=/usr/lib/llvm-11/include/
LLVM_INCLUDE_PATH=/usr/include/llvm-3.6/
 
#LLVM_PATH = /usr/lib/llvm-11
...
# kernel modules build default = false
...
.PHONY: build prepare-dirs core reporters filters query analyzers clients utilities storages screens android-build
...
# No sketches at the moment available
#build-java: download-neo4j core clients utilities storages filters query analyzers sketches transformers reporters
build-java: download-neo4j core clients utilities storages screens filters query analyzers transformers reporters
...
storages:
	...
	@echo "--- Built Storages ---"
screens:
	$(JAVAC) $(JAVAC_OPTIONS) -cp '$(JAVAC_CP)' -sourcepath src -d build src/spade/screen/*.java
	@echo "--- Built Screens ---"
utilities:
...
build-linuxaudit: build/spade/reporter/Audit.class bin/spadeAuditBridge audit-kernel-module
		...
		if [ "$(KERNEL_MODULES)" = "true" ]; then \
			if [ -z "$(OS_KERNEL_NUMBER)" ]; then \
				...
				if [ $(OS_KERNEL_NUMBER) -ge $(MAX_OS_KERNEL_NUMBER) ]; then \
					echo 'Warning: Kernel version > 5.7 uses workaround from https://github.com/xcellerator/linux_kernel_hacking/issues/3#issue-782815541 by hkerma'; \
				else \
